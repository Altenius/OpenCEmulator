...
ManageComponents::ManageComponents(QWidget *parent) : QWidget(parent), m_ui(new Ui::ManageComponents), m_modifier(nullptr) {
    m_ui->setupUi(this);
...
void ManageComponents::componentsChanged(const std::vector<ComponentPtr> &components) {
    ...
    for (const ComponentPtr &component : components) {
        QListWidgetItem *item = new QListWidgetItem(QString::fromStdString(component->label() + " (" + component->type() + ")"), m_ui->listComponents);
