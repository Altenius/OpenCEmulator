...
public:
    ...
    inline void detachKeyboard(const ComponentPtr &keyboard) {
        std::remove_if(m_keyboards.begin(), m_keyboards.end(), [keyboard] (ComponentWeakPtr &c) {return c.lock() == keyboard; });
    }
    
    inline void detachKeyboard(ComponentKeyboard *keyboard) {
        std::remove_if(m_keyboards.begin(), m_keyboards.end(), [keyboard] (ComponentWeakPtr &c) {return c.lock().get() == keyboard; });
    }
