...
do
    ...
    local function match_capture(ms, s, l)
        ...
        if ms.src_end - s >= len and
                ms.capture[l].init:head(len) == s:head(len) then
            return s:copy(len)
        else return nil
        end
    end
    ...
    function match(ms, s, p)
        s = s:copy()
        p = p:copy():: init:: -- using goto's to optimize tail recursion
        if p ~= ms.p_end then
        local p0 = p:char()
        ...
        elseif p0 == L_ESC then -- escaped sequences not in the format class[*+?-]?
            ...
            elseif p1 == 'f' then -- frontier?
                ...
                if not matchbracketclass(previous, p, ep:copy(-1)) and
                        matchbracketclass(s:char(), p, ep:copy(-1)) then
                    p = ep
...
if not ret[1] then error(ret[2], 0)
end
...
-- in the load methods of the persisted userdata wrappers (see below).[persistKey and persistKey() or "LuaJ"] = function()
return function()
...
-- Collect wrapped callback methods.[persistKey and persistKey() or "LuaJ"] = function(self)
